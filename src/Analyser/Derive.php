...
namespace Triangle\WebAnalyzer\Analyser;

use Triangle\WebAnalyzer\Constants;
use Triangle\WebAnalyzer\Model\Family;
use Triangle\WebAnalyzer\Model\Using;
use Triangle\WebAnalyzer\Model\Version;
...
trait Derive
{
    ...
    private function &deriveDeviceSubType()
    {
        if ($this->data->device->type == 'mobile' && empty($this->data->device->subtype)) {
            ...
            if (isset($this->data->os->family) && $this->data->os->family->getName() == 'Android') {
                $this->data->device->subtype = 'smart';
            }
        }
